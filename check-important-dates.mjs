...
// de fútbol y eventos en el Movistar Arena (antes WiZink Center) que puedan llenar el restaurante,
...
al Movistar Arena (el antiguo WiZink Center, en la calle Goya), a anticipar
los días con más afluencia para reforzar la producción de cocina.

Busca información actualizada (usa siempre la búsqueda web, no te fíes solo
de tu memoria) sobre PARTIDOS DE FÚTBOL y EVENTOS EN EL MOVISTAR ARENA
...
Un EVENTO EN EL MOVISTAR ARENA (concierto, boxeo u otro espectáculo) es
importante si:
...
- El recinto se llamaba antes "WiZink Center" y ahora "Movistar Arena": es
  el MISMO sitio. En los títulos escribe siempre "Movistar Arena", y no
  repitas un mismo evento porque unas fuentes usen un nombre y otras el otro.
...
[
  {
    ...
    "event_time": "HH:MM, o el valor null (sin comillas) si no se sabe",
    "title": "texto corto, ej. 'Real Madrid - Barcelona' o 'X en Movistar Arena'",
...
// Misma lógica que la importación de CSV de la app (js/app.js): los recintos
// renombrados por patrocinio cuentan como el mismo sitio, y "A - B" es lo
// mismo que "B vs A".
const VENUE_ALIASES = [
...
function canonicalTitle(title) {
  const t = VENUE_ALIASES.reduce((acc, [re, to]) => acc.replace(re, to), normalize(title));
  return t.split(/\s*(?:-|vs\.?|v\.)\s*/).map(p => p.trim()).filter(Boolean).sort().join('|');
}
// Clave de duplicado: fecha + título canónico (sin la categoría: Gemini a
// veces llama "concierto" y otras "evento" a lo mismo).
const eventKey = ev => `${ev.event_date}|${canonicalTitle(ev.title)}`;

// Hora válida "HH:MM" o null. Gemini a veces devuelve "null" como texto o
// "21:00h": antes eso hacía fallar el guardado de TODOS los eventos del día.
...
// Hoy en Madrid, AAAA-MM-DD (para descartar fechas pasadas)
...
async function main() {
  ...
  const newEvents = events.filter(ev => {
    if (!ev || !ev.title || !/^\d{4}-\d{2}-\d{2}$/.test(ev.event_date || '')) return false; // fecha con formato válido
    if (ev.event_date < today) return false;                                               // nada del pasado
    const k = eventKey(ev);
    if (seen.has(k)) return false;   // ya estaba guardado, o repetido en la propia respuesta
    seen.add(k);
    return true;
  });
  ...
  const rows = newEvents.map(ev => ({
    ...
    event_time: cleanTime(ev.event_time),
    title:      ev.title,
